...
<Aside type="caution" title="Dev-only runtime check">
In non-production (`NODE_ENV !== 'production'`), a returned body that violates its declared
schema fails loudly — types don't catch an `any` coming from the database. There is **zero**
response validation in production (for performance).
</Aside>
